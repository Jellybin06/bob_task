...
void recvThread(int sd) {
	...
	char buf[BUFSIZE];
	while (true) {
		ssize_t resV = ::recv(sd, buf, BUFSIZE - 1, 0);
		if (resV == 0 || resV == -1) {
			fprintf(stderr, "recv return %zd", resV);
			myerror(" ");
			break;
		}
		buf[resV] = '\0';
		...
		fflush(stdout);
		if (param.echo) {
			ssize_t res = ::send(sd, buf, resV, 0);
			if (res == 0 || res == -1) {
		...
		if (param.bc) {
			...
			for (auto it = clients.begin(); it != clients.end(); ++it) {
				int clientSd = *it;
				if (clientSd == sd) continue;
				ssize_t sres = ::send(clientSd, buf, resV, 0);
				if (sres == 0 || sres == -1) {
					fprintf(stderr, "send return %zd", sres);
					myerror(" ");
					break;
				}
			}
